...
export default function convert(
  ...
  const {withSource = false, excludeService = false, outputDir = 'flow-output', commonPath} = options;
  const allOutput = {};
  for (const thriftPath of thriftPaths) {
    const converter = new ThriftFileConverter(thriftPath, withSource, excludeService);
    converter
      .getImportAbsPaths()
